...
// 2. Wait for all images inside a specific element to load
async function waitForImagesInContainer(container) {
    ...
    const imagePromises = images.map((img) => {
        ...
        return new Promise((resolve) => {
            ...
            img.addEventListener('error', resolve); // Resolve even if it fails
        });
    });
...
async function initLandingPage() {
    ...
    const container = await waitForContainer('.landing-content');
    
    console.log('Container found, waiting for images...');
    await waitForImagesInContainer(container);
    
    console.log('All images loaded! Starting transition.');
    
    // Your existing fade-out logic
