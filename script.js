document.addEventListener("DOMContentLoaded", () => {
  ...
  // Isolate the heavy assets we want to wait for
  const heavyZone = document.querySelector('.heavy-asset');
  const heavyImages = heavyZone ? heavyZone.querySelectorAll('img, video') : [];
  
  let loadedCount = 0;
  const totalAssets = heavyImages.length;
  ...
  if (totalAssets === 0) {
    ...
    heavyImages.forEach(img => {
      if (img.complete) {
        ...
        img.addEventListener('error', assetLoaded); // Don't hang forever if an image breaks
      }
    });
  }
  ...
  function revealPage() {
    // 1. Fade out the loader screen
    loader.style.opacity = '0';
    loader.style.visibility = 'hidden';
    
    // 2. Restore normal scrolling to the page
    document.body.classList.remove('loading-active');
    
    // 3. Smoothly animate the page content into view
    mainContent.classList.add('page-ready');
    
    // 4. Clean up DOM after transition finishes to save memory
    setTimeout(() => {
      loader.remove(); 
    }, 500);
  }
